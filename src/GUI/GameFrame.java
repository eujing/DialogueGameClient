...
public class GameFrame extends JFrame {
	...
	public GameFrame (final String ipAddress, final short port) {
	...
	private void initialize (String ipAddress, short port) {
		this.setLookAndFeel ();
		this.setLayout (new BorderLayout ());

		String name = this.getPlayerName ();
		this.gEngine = new GameEngine (name, this.getPlayerAvatar (), ipAddress, port);
	...
	public static void main (String[] args) {
		...
		for (String arg : args) {
			...
			else if (arg.equals ("-teacher")) {
				GameEngine.PLAYER_TYPE = PlayerType.TEACHER;
			}
			else if (arg.startsWith ("-ip")) {
				arg = arg.replace (" ", "");
				ipAddress = arg.replace ("-ip=", "");
			}
